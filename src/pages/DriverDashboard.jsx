...
// Mock Recent Activity Trips
const recentTrips = [
  { name: "Sarah M.", time: "10 mins ago", amount: "৳320", status: "Completed", type: "Ride", tip: "+৳30", pickup: "Gulshan-2 Circle", dropoff: "Banani Super Market", phone: "+880 1711-223344", account: "MEC-8842-991" },
  { name: "David L.", time: "1 hr ago", amount: "৳550", status: "Completed", type: "Ride", tip: "+৳50", pickup: "Dhanmondi 27", dropoff: "Bashundhara R/A", phone: "+880 1819-556677", account: "MEC-3321-445" },
  { name: "Emma W.", time: "2 hrs ago", amount: "৳210", status: "Completed", type: "Ride", pickup: "Mohakhali DOHS", dropoff: "Tejgaon Industrial Area", phone: "+880 1912-998877", account: "MEC-1192-334" },
  { name: "Mike T.", time: "4 hrs ago", amount: "৳800", status: "Completed", type: "Delivery", tip: "+৳100", pickup: "Uttara Sector 11", dropoff: "Motijheel Shapla Chattar", phone: "+880 1722-112233", account: "MEC-5544-221" },
  { name: "Jessica R.", time: "5 hrs ago", amount: "৳420", status: "Completed", type: "Ride", tip: "+৳40", pickup: "Mirpur 10", dropoff: "Farmgate", phone: "+880 1611-334455", account: "MEC-7766-554" },
  { name: "Alex B.", time: "6 hrs ago", amount: "৳250", status: "Completed", type: "Ride", pickup: "Badda Link Road", dropoff: "Rampura Bridge", phone: "+880 1555-667788", account: "MEC-2233-445" },
  { name: "Rafiq A.", time: "7 hrs ago", amount: "৳380", status: "Completed", type: "Ride", tip: "+৳20", pickup: "Khilgaon Taltola", dropoff: "Mogbazar", phone: "+880 1777-889900", account: "MEC-9988-776" },
  { name: "Nadia K.", time: "7.5 hrs ago", amount: "৳150", status: "Completed", type: "Ride", pickup: "Shyamoli Square", dropoff: "Kalyanpur", phone: "+880 1888-990011", account: "MEC-4455-667" },
  { name: "Farhan S.", time: "8 hrs ago", amount: "৳900", status: "Completed", type: "Delivery", tip: "+৳150", pickup: "Purbachal", dropoff: "Kawran Bazar", phone: "+880 1999-001122", account: "MEC-1122-334" },
  { name: "Tasnim Z.", time: "9 hrs ago", amount: "৳400", status: "Completed", type: "Ride", pickup: "Mohammadpur Bus Stand", dropoff: "Azimpur", phone: "+880 1333-445566", account: "MEC-6677-889" },
  { name: "Karim H.", time: "10 hrs ago", amount: "৳620", status: "Completed", type: "Ride", tip: "+৳50", pickup: "Savar", dropoff: "Gabtoli", phone: "+880 1444-556677", account: "MEC-3344-556" },
  { name: "Sonia P.", time: "11 hrs ago", amount: "৳280", status: "Completed", type: "Ride", pickup: "Eskaton", dropoff: "Shantinagar", phone: "+880 1666-778899", account: "MEC-8899-001" }
];
...
export default function DriverDashboard() {
  ...
  return (
    <>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-baseline mb-0.5">
                            <h4 className={`text-sm truncate transition-all duration-300 group-hover:scale-105 group-hover:text-red-400 ${passenger.unread ? 'font-bold text-white' : 'font-medium text-gray-200'}`}>{passenger.name}</h4>
              ...
              <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
                {recentTrips.map((trip, i) => (
                  <div 
                    ...
                    className="flex items-center justify-between p-4 rounded-2xl bg-black/40 border border-white/5 hover:bg-white/10 transition-all duration-300 group cursor-pointer relative overflow-hidden"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-gray-800 to-gray-700 flex items-center justify-center font-black text-sm border border-white/10 group-hover:border-red-500/50 group-hover:shadow-[0_0_15px_rgba(239,68,68,0.3)] transition-all">
                        {trip.name[0]}
                      </div>
                      <div>
                        {/* Name Hover Animation added here */}
                        <h4 className="font-bold text-gray-200 group-hover:text-red-400 group-hover:scale-105 origin-left transition-all duration-300 text-sm">{trip.name}</h4>
                        <p className="text-[10px] text-gray-500 font-medium">{trip.time} • {trip.type}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="font-black text-md text-white mb-0.5 group-hover:scale-105 origin-right transition-transform">{trip.amount}</div>
                      {trip.tip ? (
      ...
      {selectedTrip && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn" onClick={() => setSelectedTrip(null)}>
          <div 
            className="bg-[#0a0a0a] border border-white/10 rounded-3xl p-8 max-w-md w-full shadow-[0_0_50px_rgba(0,0,0,0.8)] relative"
            onClick={(e) => e.stopPropagation()} // Prevent clicking inside modal from closing it
          >
            {/* Close Button */}
            <button 
              onClick={() => setSelectedTrip(null)} 
              className="absolute top-5 right-5 w-8 h-8 flex items-center justify-center rounded-full bg-white/5 hover:bg-red-500/20 text-gray-400 hover:text-red-400 transition-colors"
            >
            ...
            <div className="flex items-center gap-4 mb-8">
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-red-600 to-orange-500 flex items-center justify-center font-black text-2xl text-white shadow-[0_0_20px_rgba(239,68,68,0.4)]">
                {selectedTrip.name[0]}
              </div>
              <div>
                <h3 className="text-2xl font-black text-white">{selectedTrip.name}</h3>
                <p className="text-gray-400 font-medium">{selectedTrip.time} • {selectedTrip.type}</p>
              </div>
            </div>

            {/* Route Details */}
            <div className="space-y-6 mb-8 bg-white/5 rounded-2xl p-5 border border-white/5">
              <div className="flex items-start gap-4">
                <div className="mt-1 w-4 h-4 rounded-full bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.8)] ring-4 ring-blue-500/20"></div>
                <div>
                  <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1">Pickup Location</p>
                  <p className="font-semibold text-gray-200 text-lg">{selectedTrip.pickup}</p>
                </div>
              </div>
              <div className="w-0.5 h-8 bg-gradient-to-b from-blue-500 to-red-500 ml-2 opacity-50"></div>
              <div className="flex items-start gap-4">
                <div className="mt-1 w-4 h-4 rounded-full bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.8)] ring-4 ring-red-500/20"></div>
                <div>
                  <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1">Dropoff Location</p>
                  <p className="font-semibold text-gray-200 text-lg">{selectedTrip.dropoff}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-1">Fare Paid</p>
                <p className="text-2xl font-black text-white">{selectedTrip.amount}</p>
              </div>
              <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-1">Tip Included</p>
                <p className="text-2xl font-black text-yellow-400">{selectedTrip.tip || "৳0"}</p>
              </div>
              <div className="col-span-2 bg-white/5 rounded-2xl p-4 border border-white/5">
                ...
                <p className="text-lg font-bold text-gray-200 tracking-wider">{selectedTrip.phone}</p>
              </div>
              <div className="col-span-2 bg-white/5 rounded-2xl p-4 border border-white/5 flex items-center justify-between">
                <div>
                  <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-1">Mechify Account</p>
                  <p className="text-sm font-bold text-gray-400 font-mono tracking-widest">{selectedTrip.account}</p>
                </div>
            ...
            <button 
              onClick={() => setSelectedTrip(null)}
              className="w-full py-4 rounded-xl bg-gradient-to-r from-gray-800 to-gray-700 hover:from-gray-700 hover:to-gray-600 text-white font-bold uppercase tracking-widest text-sm transition-colors border border-white/10"
            >
              Close Details
            </button>
          </div>
